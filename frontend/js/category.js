...
function fetchCategoryProducts(type, gender) {
    ...
    fetch(`${url}/jewelry`)
        ...
        .then(data => {
            ...
            const filteredProducts = data.jewelry.filter(item => {
...
function setupFilters() {
    ...
    if (sortFilter) {
        sortFilter.addEventListener('change', function() {
            ...
            // Trier les produits selon le filtre sélectionné
            switch(this.value) {
                case 'price-low':
                    products.sort((a, b) => {
                        const priceA = parseFloat(a.querySelector('.price').textContent.match(/\d+\.\d+/)[0]);
                        const priceB = parseFloat(b.querySelector('.price').textContent.match(/\d+\.\d+/)[0]);
            ...
            // Vider et réinsérer les produits dans le nouvel ordre
            container.innerHTML = '';
            products.forEach(product => {
